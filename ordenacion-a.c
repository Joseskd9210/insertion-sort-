...
//prototipos
void insertion_sort(char* array, int cantidad, char* ordenacion); // ordenacion por insercion directa 
...
int main (void)
{
	...
	while (fgets(linea, TAMANOLINEA, fp) != NULL) {
		...
		while (p != NULL) {
			...
			if (numero_palabras_segmento >= CRECIMIENTO) { // limite extendemos palabras
				...
				pal2 = (char*)malloc(CRECIMIENTO * TAMPALABRAS * numero_segmento);
				memcpy(pal2, palabras, CRECIMIENTO * TAMPALABRAS * (numero_segmento - 1));
				free(palabras);
				palabras = pal2;
				pal = palabras + TAMPALABRAS * (numero_palabras - 1);
	...
	insertion_sort(palabras, numero_palabras, ordenacion);
...
// Esta rutina ordena utilizando el método de inserción directa


void insertion_sort (char* palabras, int cantidad, char* ordenacion)
{
	char temp[TAMPALABRAS];	//array de intercambio 
	int i, j;
